...
test("GET /user/{userID}/businesses - Get businesses by invalid category", async t => {
    const { body, statusCode } = await t.context.got("user/1/businesses?category-name=InvalidCategory", {
...
test("GET /user/{userID}/businesses/search - Search business by invalid keyword", async t => {
    const {body, statusCode} = await t.context.got("user/1/businesses/search?keyword=InvalidKeyword", {
        throwHttpErrors: false
    });
    t.is(statusCode, 404);
});
...
test("GET /user/{userID}/businesses - Get businesses by invalid user ID", async t => {
    const {body, statusCode} = await t.context.got("user/999/businesses?category-name=breakfast", {
        throwHttpErrors: false
    });
    t.is(statusCode, 404);
});   